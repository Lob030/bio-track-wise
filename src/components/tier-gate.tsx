    ...
    <div className="p-8 flex items-center justify-center min-h-[60vh]">
      <Card className="max-w-md p-8 text-center border-amber-700/40 bg-card/60 backdrop-blur">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Tu plan <span className="capitalize font-semibold text-foreground">{tier}</span> no
          incluye este módulo. Requiere plan{" "}
          <span className="capitalize font-semibold text-primary">{min}</span> o superior.
        </p>
        <Button asChild className="w-full">
          <Link to="/billing">
            <Sparkles className="h-4 w-4 mr-2" /> Ver planes
          </Link>
        </Button>
      </Card>
    </div>
