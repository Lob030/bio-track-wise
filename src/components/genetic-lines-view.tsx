...
function unpackMeta(raw: string | null) {
  ...
  try {
    ...
    return { date: o.date ?? "", origin: o.origin ?? "", notes: o.notes ?? "" };
  } catch {
    return { date: "", origin: "", notes: raw };
  }
}
...
export function GeneticLinesView({ kind }: { kind: Kind }) {
  ...
  const submit = async () => {
    if (!form.name.trim() || !form.species_id)
      return toast.error("Nombre y especie son requeridos");
  ...
  const importCSV = async () => {
    ...
    try {
      rows = await pickCSVFile();
    } catch {
      return toast.error("No fue posible leer el archivo CSV.");
    }
    if (!rows.length) return;
    const operation = `import:genetic-lines:${kind}`;
    const { data, error } = await operationsDb.rpc("import_genetic_lines_tx", {
      _request_id: getRequestId(operation),
      _kind: kind,
      _rows: rows,
    });
    if (error) return toast.error(`${toUserFriendlyError(error)} No se importó ninguna fila.`);
    resetRequestId(operation);
    toast.success(`${Number((data as { imported?: number })?.imported ?? 0)} líneas importadas`);
    qc.invalidateQueries({ queryKey: ["lines", kind] });
  };
          ...
          <AdminOnly>
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    {editingLine ? "Editar línea genética" : "Nueva línea genética"}
                  </DialogTitle>
                </DialogHeader>
                <div className="grid gap-3">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setForm({ ...form, species_id: v })}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Proveedor / colonia origen"
                    />
                  </div>
                  <div>
                    <Label>Notas</Label>
                ...
                <DialogFooter>
                  <Button onClick={submit}>{editingLine ? "Guardar cambios" : "Registrar"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </AdminOnly>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
        {searchedLines.length === 0 && (
          <Card className="col-span-full p-10 text-center text-muted-foreground border-dashed">
            Aún no hay líneas genéticas registradas.
          </Card>
        )}
        {searchedLines.map((l) => {
