...
export const Route = createFileRoute("/insects/species")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Configura las especies de insectos registradas en BioTrack.",
      },
...
function Page() {
  ...
  const [rules, setRules] = useState<InsectRule[]>([
    { label: "", min_days: 0, max_days: 0, individuals_per_gram: 0, price_mxn: 0 },
  ]);
  ...
  const submit = async () => {
    ...
    const invalidRule = filteredRules.some(
      (r) =>
        [r.min_days, r.max_days, r.individuals_per_gram, r.price_mxn].some(
          (value) => !Number.isFinite(Number(value)) || Number(value) < 0,
        ) ||
        r.max_days < r.min_days ||
        Number(r.individuals_per_gram) <= 0,
    );
    if (invalidRule) {
      return toast.error("Revisa las tallas: los rangos, densidad y costos deben ser validos");
    }
        ...
        <AdminOnly>
          <>
            <Button
              ...
              onClick={() => create("Tenebrios", TENEBRIOS)}
            >
                ...
                <div className="flex flex-col gap-5 overflow-hidden min-h-0 flex-1">
                  ...
                  <div className="flex flex-col min-h-0 flex-1">
                    ...
                    <div className="grid grid-cols-12 gap-2 text-[11px] uppercase text-muted-foreground/80 px-2 shrink-0 font-bold">
                      <span className="col-span-3">Etapa</span>
                      <span className="col-span-2">Día min</span>
                      <span className="col-span-2">Día max</span>
                      <span className="col-span-2">Ind. / 1g</span>
                    ...
                    <div className="space-y-2 overflow-y-auto flex-1 mt-1 pr-1">
                      {rules.map((r, i) => (
                        <div key={i} className="grid grid-cols-12 gap-2 items-center">
                          <Input
                            className="col-span-3 h-10 focus-visible:ring-2 focus-visible:ring-primary"
                            value={r.label}
                            onChange={(e) => {
                              const n = [...rules];
                          ...
                          <Input
                            className="col-span-2 h-10 focus-visible:ring-2 focus-visible:ring-primary"
                            type="number"
                            value={r.individuals_per_gram}
                            onChange={(e) => {
                              const n = [...rules];
                              n[i].individuals_per_gram = +e.target.value;
                              setRules(n);
                            }}
                          />
                          <Input
