...
function Page() {
  ...
  return (
    <PageShell
      ...
      actions={
        <>
          <Button variant="outline" className="h-10 md:h-9 min-h-10 md:min-h-9 transition-all duration-200" onClick={() => create("Tenebrios", TENEBRIOS)}>
            <Wand2 className="h-5 md:h-4 w-5 md:w-4 mr-2" /> Preset Tenebrios
          </Button>
            ...
            <DialogContent className="max-w-4xl p-6 gap-6 flex flex-col max-h-[90vh]">
              <DialogHeader>
                <DialogTitle className="text-lg font-bold tracking-tight text-foreground">
                  {editingSpecies ? "Editar especie de insecto" : "Nueva especie de insecto"}
                </DialogTitle>
              </DialogHeader>
              <div className="flex flex-col gap-5 overflow-hidden min-h-0 flex-1">
                <div className="shrink-0">
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej. Grillos" className="w-full h-10 focus-visible:ring-2 focus-visible:ring-primary" />
                </div>
                <div className="flex flex-col min-h-0 flex-1">
                  <div className="flex items-center justify-between mb-2 shrink-0">
                    <Label className="text-sm font-semibold text-foreground/90">Matriz de etapas</Label>
                    <Button size="sm" variant="ghost" className="h-8 transition-all hover:bg-accent" onClick={() => setRules([...rules, { label: "", min_days: 0, max_days: 0, individuals_per_gram: 0, price_mxn: 0 }])}>+ Agregar fila</Button>
                  </div>
                  ...
                  <div className="space-y-2 overflow-y-auto flex-1 mt-1 pr-1">
                    {rules.map((r, i) => (
                      <div key={i} className="grid grid-cols-12 gap-2 items-center">
                        <Input className="col-span-3 h-10 focus-visible:ring-2 focus-visible:ring-primary" value={r.label} onChange={(e) => { const n = [...rules]; n[i].label = e.target.value; setRules(n); }} placeholder="Huevo" />
                        <Input className="col-span-2 h-10 focus-visible:ring-2 focus-visible:ring-primary" type="number" value={r.min_days} onChange={(e) => { const n = [...rules]; n[i].min_days = +e.target.value; setRules(n); }} />
                        <Input className="col-span-2 h-10 focus-visible:ring-2 focus-visible:ring-primary" type="number" value={r.max_days} onChange={(e) => { const n = [...rules]; n[i].max_days = +e.target.value; setRules(n); }} />
                        <Input className="col-span-2 h-10 focus-visible:ring-2 focus-visible:ring-primary" type="number" value={r.individuals_per_gram} onChange={(e) => { const n = [...rules]; n[i].individuals_per_gram = +e.target.value; setRules(n); }} />
            ...
            <Card key={s.id} className="border-border/50 bg-gradient-to-br from-card to-card/40 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
              <button onClick={() => setExpanded(isOpen ? null : s.id)} className="w-full flex items-center justify-between p-4 hover:bg-accent/15 transition-all duration-200">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="text-[10px] border-border/40 bg-accent/15">insecto</Badge>
              ...
              {isOpen && <div className="p-4 pt-0 border-t border-border/30 bg-accent/5"><InsectSizeMatrix rules={rs} /></div>}
            </Card>
          );
        })}
      </div>
