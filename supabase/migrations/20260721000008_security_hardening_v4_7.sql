-- 20260721000008_security_hardening_v4_7.sql
-- v4.7: Eliminar sobrecargas ambiguas de adjust_lot, prevención de auditorías vacías (no-op),
--       y RPC get_security_function_signatures restringida a service_role.

BEGIN;

-------------------------------------------------------------------
-- 1. Eliminar explícitamente cualquier firma antigua de adjust_lot
-------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.adjust_lot(UUID, INT, INT, INT, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS public.adjust_lot(UUID, INT, INT, INT, NUMERIC, TEXT, TEXT);

-------------------------------------------------------------------
-- 2. ÚNICA firma de adjust_lot (v4.7 con prevención de auditoría no-op)
-------------------------------------------------------------------
...
BEGIN
  ...
  -- 7. Evitar auditorías vacías / no-op: Si no hay ningún cambio real, salir sin modificar ni insertar historial
  IF _new_males = _old_males
     AND _new_females = _old_females
     AND _new_unsexed = _old_unsexed
     AND _new_mass = _old_mass
     AND (_new_tags IS NOT DISTINCT FROM _old_tags) THEN
    RETURN;
  END IF;

  -- 8. Determinar estado de inventario en cero (Finalización atómica)
  ...
  -- 9. Actualizar lote (CONSERVA lots.notes ORIGINAL)
  ...
  -- 10. Registrar evento de historial con event_type = 'adjustment'
  ...
  -- 11. Registrar en audit_log con audit_action = 'inventory_adjustment'
...
-------------------------------------------------------------------
-- 3. RPC de Diagnóstico get_security_function_signatures (service_role ONLY)
-------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_security_function_signatures()
RETURNS TABLE (
  proname TEXT,
  identity_args TEXT
)
...
AS $$
  ...
  WHERE n.nspname = 'public'
    AND p.proname IN ('fifo_consume_rodents', 'fifo_consume_insects', 'adjust_lot');
$$;

REVOKE EXECUTE ON FUNCTION public.get_security_function_signatures() FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.get_security_function_signatures() TO service_role;

-- Mantener la vista/función get_fifo_signatures por retrocompatibilidad
...
SET search_path = public, pg_catalog
AS $$
  SELECT * FROM public.get_security_function_signatures();
$$;
