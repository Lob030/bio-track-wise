...
-- v4.5: RPC de ajuste administrativo de lotes, validaciones FIFO mejoradas.
--
-- Correcciones:
--   1. adjust_lot RPC: Las ediciones directas de población/masa deben generar lot_events + audit_log.
--   2. fifo_consume_insects: Rechaza NULL, <=0, NaN, Infinity, -Infinity en _grams.
--   3. Validaciones de talla: talla inexistente, started_at futuro, size_rules vacías.

BEGIN;

-------------------------------------------------------------------
-- 1. adjust_lot: RPC para ajustes administrativos con trazabilidad
--    Reemplaza las ediciones directas a lots (males, females, unsexed, mass_grams, notes).
--    Genera lot_events y audit_log en la misma transacción.
-------------------------------------------------------------------
...
BEGIN
  ...
  -- Calcular nuevos valores (solo actualiza campos proporcionados)
...
-------------------------------------------------------------------
-- 2. fifo_consume_insects MEJORADA
--    - Rechaza NaN, Infinity, -Infinity en _grams
--    - Valida started_at futuro (lotes con fecha futura no se incluyen)
-------------------------------------------------------------------
...
BEGIN
  ...
  -- Validar _grams: rechazar NULL, <=0, NaN, Infinity, -Infinity
  ...
  FOR _lot IN
    SELECT l.*,
     ...
     WHERE l.organization_id = _org
       ...
       AND l.started_at <= CURRENT_DATE  -- Excluir lotes con fecha futura
       AND (
         NOT _filter_size
         OR EXISTS (
           SELECT 1
...
-------------------------------------------------------------------
-- 3. fifo_consume_rodents MEJORADA
--    - Excluye lotes con started_at futuro
--    - Validación reforzada de qty (NaN check)
-------------------------------------------------------------------
